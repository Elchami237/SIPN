            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-8">
                <form action="{{ route('admin.projects.update', $project->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-6">
                        ...
                        <input type="text" name="title[fr]" id="title_fr" value="{{ old('title.fr', $project->getTranslation('title', 'fr')) }}" required
                    ...
                    <div class="mb-6">
                        ...
                        <input type="text" name="title[en]" id="title_en" value="{{ old('title.en', $project->getTranslation('title', 'en')) }}"
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-sipn-orange focus:border-transparent">
                        @error('title.en')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="mb-6">
                        ...
                        <textarea name="description[fr]" id="description_fr" rows="5" required
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-sipn-orange focus:border-transparent">{{ old('description.fr', $project->getTranslation('description', 'fr')) }}</textarea>
                    ...
                    <div class="mb-6">
                        ...
                        <textarea name="description[en]" id="description_en" rows="5"
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-sipn-orange focus:border-transparent">{{ old('description.en', $project->getTranslation('description', 'en')) }}</textarea>
                    ...
                    <div class="mb-6">
                        ...
                        <select name="category" id="category" required
                            ...
                            <option value="construction" {{ old('category', $project->category) == 'construction' ? 'selected' : '' }}>Construction</option>
                            <option value="genie_civil" {{ old('category', $project->category) == 'genie_civil' ? 'selected' : '' }}>Génie civil</option>
                            <option value="location" {{ old('category', $project->category) == 'location' ? 'selected' : '' }}>Location</option>
                            <option value="travaux_publics" {{ old('category', $project->category) == 'travaux_publics' ? 'selected' : '' }}>Travaux publics</option>
                        </select>
                        @error('category')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    @if($project->featured_image)
                        <div class="mb-4">
                            ...
                            <img src="{{ Storage::url($project->featured_image) }}" alt="{{ $project->title }}" class="w-48 h-32 object-cover rounded-lg">
                        </div>
                    @endif

                    <!-- Featured Image -->
                    <div class="mb-6">
                        <label for="featured_image" class="block text-sm font-medium text-gray-700 mb-2">Nouvelle image mise en avant</label>
                        <input type="file" name="featured_image" id="featured_image" accept="image/jpeg,image/jpg,image/png,image/webp"
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-sipn-orange focus:border-transparent">
                        <p class="mt-1 text-sm text-gray-500">Laissez vide pour conserver l'image actuelle. Formats acceptés : JPEG, JPG, PNG, WebP (max 2 Mo)</p>
